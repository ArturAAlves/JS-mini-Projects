...
function createMenu(arr) {
    const printMenu = arr.map(function (item) {
...
function createBtn() {
    const btnReducer = menuList.reduce(function (newList, item) {
    ...
    const printBtns = btnReducer.map((btn) => {
...
function filter() {
    const btnList = btnContainer.childNodes
    btnList.forEach(btn => {
        btn.addEventListener("click", function () {
            let btnName = btn.dataset.id
            const filtered = menuList.filter(function (item) {
